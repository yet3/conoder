...
const Node: T = function (props) {
  ...
  return (
    <BaseNode {...props} node={Node} className="text-left" contentClassName='gap-4 w-64'>
      <Input containerClassName='mx-auto w-24' label="Line" type="number" min={1} value={line} onChange={(v) => setSetting('line', v as number)} />
      <CodeEditor value={code} onChange={(v) => setSetting('code', v)} />
    </BaseNode>
  );
};

Node.nodeName = 'Code: Insert at line';
...
Node.execute = async ({ settings, content, setContent }, next) => {
  ...
  if (lines.length >= line) {
    lines.splice(line - 1, 0, ...code.split('\n'));
  } else {
...
export { Node as CodeInsertAtLineNode };
